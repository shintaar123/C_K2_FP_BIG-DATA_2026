"""
Train Importance Classifier (Binary: tinggi / rendah)
...
MODEL_OUTPUT = "s3a://mlflow/models/importance_classifier"
MLFLOW_URI   = "http://mlflow:5000"
EXPERIMENT   = "importance-classifier"
MINIO_ENDPOINT   = "http://minio:9000"
MINIO_ACCESS_KEY = "minioadmin"
MINIO_SECRET_KEY = "minioadmin123"

spark = (
    SparkSession.builder.appName("train-importance-classifier")
...
print(">>> Training Importance Classifier dimulai...")

df = spark.read.csv(LABELED_CSV, header=True, inferSchema=True)
df = df.filter(
    F.col("importance_label").isin("tinggi", "rendah") &
    F.col("clean_text").isNotNull() &
    (F.length(F.col("clean_text")) > 5)
)

print(f">>> Total sampel: {df.count()}")
print("\n>>> Distribusi importance_label:")
df.groupBy("importance_label").count().show()
...
pipeline = Pipeline(stages=[
    StringIndexer(inputCol="importance_label", outputCol="label", handleInvalid="keep"),
...
with mlflow.start_run(run_name="importance-rf-v1") as run:
    print(f">>> MLflow Run ID: {run.info.run_id}")
    mlflow.log_param("num_trees", 100)
    mlflow.log_param("max_depth", 8)
    mlflow.log_param("target", "importance_label")
    ...
    pred.select(
        ...
        F.col("importance_label").alias("aktual"),
